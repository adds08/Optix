...
| --- | --- |
| `System Shell v3.dc.html` — rail groups, sidebar rows, permission gating | `apps/web/components/sti/nav-config.ts` (FIELD_NAV / DESK_NAV, `navFor`, `isFieldRole`), `apps/web/components/app-sidebar.tsx` (per-item `perm` filter) |
| `System Shell v3.dc.html` — frame, top bar, one scroll region | `apps/web/components/sti/app-shell.tsx`, `docs/changelogs/2026-08-07-app-shell-viewport-frame.md` |
| `System Shell v3.dc.html` — role / permission props, seat scope | `packages/types/src/index.ts` (`ROLES`, `PERMISSIONS`), `packages/db/src/schema/employee.ts` (`employeeProjectAssignment`, `projectTeamMember`) |
| `System Shell v3.dc.html` — dashboard feeds, metrics | `docs/14-dashboard-additions.md`, `docs/00-executive-summary.md` |
| `Tools by Jobsite Blocky.dc.html` — board, crew rig status, `embedded` mode | `apps/web/app/(app)/jobsites/page.tsx`, `apps/web/lib/rig.ts`, `docs/18-vehicle-tracking-and-map.md` |
| `FieldChat.dc.html` — conversational hand-off | `apps/web/app/(app)/chat/page.tsx`, `docs/07-conversational-layer.md` |
| Blocky design system (`tokens/`, `components/`, `guidelines/`) | `apps/web/app/globals.css` (`:root` + `.dark`, `--radius`, `.label-xs`, `.tag-num`, reserved status tokens) |
