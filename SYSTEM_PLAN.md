...
| Production readiness | `PARTIAL` | ~~No error boundaries~~ — `apps/web/app/(app)/error.tsx` and `global-error.tsx` exist. ~~No integration tests~~ — the database-backed suites in `packages/api-contracts` run router+domain+db together against real Postgres, including concurrency races. A browser E2E harness was built 2026-08-22 (STI-001) — 27 specs across five roles in `e2e/` — removed from CI on 2026-08-30, and **DELETED entirely on 2026-09-10** after its specs drifted from renamed UI; STI-122 (make it blocking) is void with it. Browser checking is the Playwright MCP now, which is per-change evidence and not coverage. `apps/api` still has its own tests. Still missing: any test under `apps/web` or `apps/mobile` themselves. Lint non-blocking: still true. |
